import uuid, requests, json, datetime, time, sys, os
...
class MTNMoneyAPI:
    ...
    def transactionState(self, transaction_id):
        headers = {
            "Authorization": "Bearer " + self.accessToken,
            "X-Target-Environment": self.environnement,
            "Ocp-Apim-Subscription-Key": self.subcriptionKey,
        }
        req = requests.get(url=self.requestToPayURL+"/"+transaction_id, headers=headers)
        if req.status_code != 200:
    ...
    def collect_payment(self, payer, amount, transactionId, note):
        login = self.login()
        if login:
            return self.requestToPay(payer, amount, transactionId, note) #success, tid
        else:
...
class OrangeMoneyAPI:
    ...
    def requesttopay(self, transaction_id, amount, note):
        ...
        body = {
            ...
            "return_url": "https://159.223.7.139/payment/success/",
            "cancel_url": "https://159.223.7.139/payment/failed/",
        ...
        self.payment_url = req.json()["payment_url"]
        self.notif_token = req.json()["notif_token"]
        return 1, self.payment_url, self.notif_token
...
#mtn_money = MTNMoneyAPI()
#if mtn_money.login():
    #mtn_money.requestToPay("2250502455611", "100", "TIDBDIZEZNUHy", "Paiment Instapay")
